...
## Miami-Dade medium R0: estimate heterogeneity in biting behaviour using negative binomial draw (Scenario 4)
...
rproc <- Csnippet("
                  ...
                  double M = rnbinom_mu(k, lambdaM);
                  double lambdaH = a * c * 1/(mum) * M * ((S - deathsS)/pop);  
                  double infected = rnbinom_mu(k, lambdaH);

                  //Rprintf(\"  %f \\t  %f \\t %f \\t %f \\n  \", transmitters, lambdaH, infected); 
...
dengue.MD <- data %>%
  pomp(t0 = 1, 
       ...
       rmeasure = rmeas,
       rinit = initlz,
       partrans = parameter_trans(logit=c("a", "k", "rhoL", "rhoT")),
       covar = covariate_table(covar, times="times"),
       accumvars = "C",       
       obsnames=c("cases"), 
       statenames = c("N", "S", "I", "R", "C", "foiH"),
       paramnames = c("a", "k", "rhoL", "rhoT",  "gamma", "mu", "br"),
...
y.out2 <- tibble(data.frame(a=mean(test2$a), k=mean(test2$k), rhoL=mean(test2$rhoL), rhoT=mean(test2$rhoT),
