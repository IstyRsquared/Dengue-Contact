...
## Miami-Dade medium R0: V->H and H->V transmission with species-specific lambda; , 24-month lasting immunity
...
## Data
R=5;M=2
travel_all <- read.csv("/Users/u1674940/Documents/Arboviruses_CDC/Florida-Incursions/output/FL_biweekly_CountyTravel.csv")
local_all <- read.csv("/Users/u1674940/Documents/Arboviruses_CDC/Florida-Incursions/output/FL_biweekly_CountyLocal.csv")
paras <- read.csv(paste0("/Users/u1674940/Documents/Arboviruses_CDC/Florida_docs/Florida_R0_lit/Mordecai_2017/output/biweekly/R0perTparas_MiamiDade_R", R, "M", M, ".csv"))
...
rproc <- Csnippet("
                  ...
                  double lambdaH = a * c * 1/(mum) * M * ((S - deathsS)/pop);  
...
NSEQ=5000 # polio up to 8000
NP= 500 # polio up to 1000
...
guesses <- sobol_design(
  ...
  upper=c(a=1, 
          ...
          rhoT=0.3),
  nseq=NSEQ) |> 
  mutate(gamma = 30.5/(24*30.5), mu = 1027/(12*100000), br = 11/(12*1000))
...
mean(test2$a); sd(test2$a)
quantile(test2$a, 0.975)
quantile(test2$a, 0.025)

y.dengue |>
  filter(logLik == max(logLik)) -> y.out
# alpha  rhoL  rhoT   gamma     mu       br   logLik
# 0.205 0.142 0.289 0.167  0.000856 0.000917  -112 # y.out 6 mths
# 0.179 0.250 0.281 0.333  0.000856 0.000917  -112 # y.out 3 mths
# 0.200 0.157 0.279 0.0417 0.000856 0.000917  -112 # y.out 24 mths

# 0.232 0.0941 0.190 0.167  0.000856 0.000917 # y.out2 6 mths
# 0.230 0.0936 0.183 0.333  0.000856 0.000917 # y.out2 3 mths
# 0.230 0.0892 0.184 0.0417 0.000856 0.000917 # y.out2 24 mths
...
alldat <- list(data, sims_summed)
write_rds(alldat, paste0("output/sims/allcases", range, ".rds"))
