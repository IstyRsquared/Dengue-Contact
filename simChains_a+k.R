## Name: Isty Rysava
## Date: 5/6/2024
## Code: Simulate transmission chains for FL counties - a + k
...
for (idx in 1:length(names)){
  ...
  for(i in 1:length(bseas)){
    out <- replicate(1000, chain_a_k(gens = 7, I = I, a=a, bseas=bseas[i], cseas=cseas[i], 
                                        mumseas=mumseas[i], PDRseas=PDRseas[i], pop=pop, rm=rm, k=k))
...
save(outbreaks, file="./output/county_chains_a+k.Rdata")
