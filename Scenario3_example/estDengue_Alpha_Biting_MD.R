...
## Miami-Dade medium R0: V->H and H->V transmission with species-specific lambda + alpha saturation (Scenario 3)
...
rproc <- Csnippet("
                  ...
                  double seasbet = (R_pow(a,2)*b*c*exp(-mum/(PDR+ec)))/(mum*rm);
                  double lambdaH = seasbet * R_pow(transmitters, alpha)/pop;  
                  double infected = rpois((lambdaH* (S - deathsS)));
...
dengue.MD <- data %>%
  pomp(t0 = 1, 
       ...
       rmeasure = rmeas,
       rinit = initlz,
       partrans = parameter_trans(logit=c("a", "alpha", "rhoL", "rhoT")),
       covar = covariate_table(covar, times="times"),
       accumvars = "C",       
       obsnames=c("cases"), 
       statenames = c("N", "S", "I", "R", "C", "foiH"),
       paramnames = c("a", "alpha", "rhoL", "rhoT",  "gamma", "mu", "br"),
...
y.out2 <- tibble(data.frame(a=mean(test2$a), alpha=mean(test2$alpha), rhoL=mean(test2$rhoL), rhoT=mean(test2$rhoT),
