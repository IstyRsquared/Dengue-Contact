## Name: Isty Rysava
## Date: 5/6/2024
## Code: Simulate transmission chains for FL counties - a
...
for (idx in 1:length(names)){
  ...
  for(i in 1:length(bseas)){
    out <- replicate(1000, chain_a(gens = 7, I = 1, a=a, bseas=bseas[i], cseas=cseas[i], 
                                  mumseas=mumseas[i], PDRseas=PDRseas[i], pop=pop, rm=rm))
    mat[i,2:3] <- rowMeans(out)
    mat[i,4] <- quantile(out[1,], 0.975)
    mat[i,5] <- quantile(out[1,], 0.025)
    mat[i,6] <- quantile(out[2,], 0.975)
    mat[i,7] <- quantile(out[2,], 0.025)
...
save(outbreaks, file="./output/county_chains_a.Rdata")
