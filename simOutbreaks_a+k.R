## Name: Isty Rysava
## Date: 30/05/2024
## Code: Simulate outbreaks for FL counties - with a + k

rm(list=ls())

## Data
pops <- read.csv("data/county_pops.csv")
params <- read.csv("output/variables/FL_summaries_a+k.csv")
...
for (idx in 1:length(names)){
  ...
  k <- params$meank[which(params$county==names[idx])]
  ...
  pdf(paste0("figs/outbreaks_a+k/", names[idx], ".pdf"), height=8, width=6)
  par(mfrow=c(3,2))
  for(i in 1:length(bseas)){
    lambdaM = (a^2 * cseas[i] * bseas[i] * cseas[i] * exp(-mumseas[i]/(PDRseas[i]+ec)))/(rm*mumseas[i])
    lambdaM = (a * bseas[i] * exp(-mumseas[i]/(PDRseas[i]+ec)))/(rm) * I
    M = rnbinom(1000, size=k, mu=lambdaM)
    lambdaH = a * cseas[i] * 1/(mumseas[i]) * M * ((pop-1)/pop)
    mat[,i] = rnbinom(1000, size=k, mu=lambdaH)
    barplot(table(mat[,i])/1000, main=paste0(seasons[i], " a = ", round(a, digits=2), " k = ", round(k, digits=2)),
...
save(outbreaks, file="./output/county_outbreaks_a+k.Rdata")
...
pdf("figs/outbreaks_a+k/SecCases_season.pdf", height=5, width=9)
...
pdf("figs/outbreaks_a+k/SecCases_year.pdf", height=8, width=10)
