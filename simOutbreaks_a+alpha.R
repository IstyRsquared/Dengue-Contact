## Name: Isty Rysava
## Date: 21/05/2024
## Code: Simulate outbreaks for FL counties - with a + alpha
...
for (idx in 1:length(names)){
  ...
  pdf(paste0("figs/outbreaks_a+alpha/", names[idx], ".pdf"), height=8, width=6)
  par(mfrow=c(3,2))
  for(i in 1:length(bseas)){
    seasbet = (a^2 * cseas[i] * bseas[i] * cseas[i] * exp(-mumseas[i]/(PDRseas[i]+ec)))/(rm*mumseas[i])
    lambdaH = seasbet * I^alpha * (pop-I)/pop
    mat[,i] = rpois(1000, lambdaH)
    barplot(table(mat[,i])/1000, main=paste0(seasons[i], " a = ", round(a, digits=2), " alpha = ", round(alpha, digits=2)),
...
save(outbreaks, file="./output/county_outbreaks_a+alpha.Rdata")
...
pdf("figs/outbreaks_a+alpha/SecCases_season.pdf", height=5, width=9)
...
pdf("figs/outbreaks_a+alpha/SecCases_year.pdf", height=8, width=10)
